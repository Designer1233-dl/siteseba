...
// The deployment is intentionally single-account: the only account is the administrator.
// Keep the credentials out of the client; the password is stored only as a scrypt hash.
...
// Remove any accounts left by an earlier/demo deployment and provision exactly one admin.
// This also invalidates old sessions so deleted accounts cannot keep using the API.
function ensureAdminAccount() {
  ...
  const changed = users.length !== 1 || !current || current.id !== admin.id || current.role !== 'admin' || !passwordValid;
  writeUsers([admin]);
  if (changed) {
...
async function api(req,res,url){
  if(req.method==='POST' && url.pathname==='/api/register'){
    return json(res,403,{error:'Регистрация закрыта. Войдите под аккаунтом администратора.'});
  }
