...
async function api(req,res,url){
  ...
  if(req.method==='GET' && url.pathname==='/api/conversations'){
    ...
    const messages=readMessages().filter(m=>!m.toType&&(m.from===me.id||m.to===me.id)), ids=[...new Set(messages.map(m=>m.from===me.id?m.to:m.from))], users=readUsers();
