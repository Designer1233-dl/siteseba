...
async function api(req,res,url){
  ...
  if(req.method==='POST' && url.pathname==='/api/messages'){
    ...
    const messages=readMessages(),message={id:crypto.randomUUID(),from:me.id,to:other.id,text,createdAt:Date.now()};messages.push(message);writeMessages(messages);return json(res,201,{message:{...message,mine:true}});
  }
