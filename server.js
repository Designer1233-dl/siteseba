...
const json = (res, code, value) => { res.writeHead(code, {'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store','X-App-Build':APP_BUILD}); res.end(JSON.stringify(value)); };
...
const latestActivityForUser = id => {
  const direct = readMessages().filter(m => m.from === id || m.to === id).reduce((n,m) => Math.max(n,m.createdAt || 0), 0);
  const memberships = readCommunities().filter(c => (c.members || []).includes(id) || c.ownerId === id).map(c => c.id);
  return readMessages().filter(m => m.toType === 'community' && memberships.includes(m.to)).reduce((n,m) => Math.max(n,m.createdAt || 0), direct);
};
...
async function api(req,res,url){
  if(req.method==='GET' && url.pathname==='/api/health') return json(res,200,{ok:true,build:APP_BUILD,features:['accounts','communities','messages','verification','read-receipts']});
  ...
  if(communityMatch){
    ...
    if(action==='messages'&&req.method==='GET'){const users=readUsers(),stored=readMessages();let changed=false;stored.forEach(m=>{if(m.toType==='community'&&m.to===c.id&&m.from!==me.id&&!m.readAt){m.readAt=Date.now();changed=true}});if(changed)writeMessages(stored);const all=stored.filter(m=>m.toType==='community'&&m.to===c.id).slice(-300).map(m=>({...m,mine:m.from===me.id,fromUser:publicUser(users.find(u=>u.id===m.from)||{id:m.from,name:'Удалённый пользователь',username:'deleted'})}));return json(res,200,{community:{...publicCommunity(c),isMember:c.members.includes(me.id)},messages:all})}
    if(action==='messages'&&req.method==='POST'){if(!c.members.includes(me.id)&&c.type==='group')return json(res,403,{error:'Сначала вступите в группу'});const p=await body(req),text=String(p.text||'').trim(),image=String(p.image||'');if((!text&&!image)||text.length>4000)return json(res,400,{error:'Сообщение пустое'});const messages=readMessages(),replyTo=String(p.replyTo||'');const m={id:crypto.randomUUID(),from:me.id,to:c.id,toType:'community',text,image,viewOnce:Boolean(p.viewOnce&&image),replyTo:replyTo||null,createdAt:Date.now(),readAt:null};messages.push(m);writeMessages(messages);c.members.forEach(wakeUser);return json(res,201,{message:{...m,mine:true}})}
  }
  ...
  if(req.method==='POST' && url.pathname==='/api/moderation/action'){
    ...
    if(action==='verifyCommunity'){
      const communities=readCommunities(), target=communities.find(c=>c.id===String(p.communityId||'')); if(!target)return json(res,404,{error:'Сообщество не найдено'});
